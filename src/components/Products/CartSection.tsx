...
function CartSection({
  ...
  return (
    <div className="sticky top-6 flex flex-col gap-6 rounded-xl bg-slate-800 p-6 shadow-lg">
      ...
      {totalItems > 0 && <hr className="border-slate-700" />}
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex justify-between text-lg font-bold text-slate-100">
          ...
          <span className="text-sky-400">${totalPrice ?? 0}</span>
        </div>
        <button
          disabled={totalItems === 0}
          onClick={onClearCart}
          className="mt-2 cursor-pointer rounded-lg border border-red-500/20 bg-red-500/10 py-2 text-sm font-medium text-red-400 transition hover:bg-red-500/20 disabled:cursor-not-allowed"
        >
          Clear Cart
        </button>
      </div>
